import { checkAdmin, createBackup, createRoom, deleteMedia, deleteRoom, listBackups, listMedia, listRooms, loadSiteData, saveSiteData, uploadMedia } from "./api.js";
...
const refreshMediaBtn = document.querySelector("#refreshMediaBtn");
const mediaList = document.querySelector("#mediaList");
const createBackupBtn = document.querySelector("#createBackupBtn");
const refreshBackupsBtn = document.querySelector("#refreshBackupsBtn");
...
function mediaPreviewCard(file) {
  if (/video\//.test(file.type)) {
    return `<video src="${escapeAttr(file.url)}" controls muted></video>`;
  }
  if (/image\//.test(file.type)) {
    return `<img src="${escapeAttr(file.url)}" alt="">`;
  }
  return `<div class="media-file-icon">文件</div>`;
}

function renderMedia(files) {
  if (!files.length) {
    mediaList.innerHTML = `<div class="empty-entry">暂无上传文件。</div>`;
    return;
  }
  mediaList.innerHTML = files.map((file) => `
    <article class="media-item">
      <div class="media-thumb">${mediaPreviewCard(file)}</div>
      <div class="media-info">
        <strong>${escapeText(file.name)}</strong>
        <span>${escapeText(formatBytes(file.size))} · ${escapeText(file.updatedAt)}</span>
        <code>${escapeText(file.url)}</code>
      </div>
      <button class="danger" type="button" data-delete-media="${escapeAttr(file.name)}">删除文件</button>
    </article>
  `).join("");
}
...
async function refreshMedia(silent = false) {
  ...
  renderMedia(await listMedia(token));
  if (!silent) setStatus("媒体列表已刷新。", "ok");
}

async function deleteMediaFromAdmin(name) {
  const token = currentToken();
  if (!token) {
    setStatus("请先登录后台。", "error");
    return;
  }
  const confirmed = window.confirm(`确定删除上传文件 ${name} 吗？如果前台仍在使用这个文件，图片或视频会失效。`);
  if (!confirmed) return;
  await deleteMedia(name, token);
  await refreshMedia(true);
  setStatus("媒体文件已删除。", "ok");
}

function renderBackups(backups) {
  if (!backups.length) {
    backupList.innerHTML = `<div class="empty-entry">暂无备份。</div>`;
    return;
  }
  backupList.innerHTML = backups.map((backup) => `
    <article class="backup-item">
      <div>
        <strong>${escapeText(backup.name)}</strong>
        <span>${escapeText(backup.createdAt)} · ${escapeText(formatBytes(backup.size))}</span>
        <code>${escapeText(backup.path)}</code>
      </div>
    </article>
  `).join("");
}
...
async function refreshBackups(silent = false) {
  ...
  renderBackups(await listBackups(token));
  if (!silent) setStatus("备份列表已刷新。", "ok");
}
...
async function createBackupFromAdmin() {
  ...
  const confirmed = window.confirm("确定现在创建备份吗？会复制数据库和上传文件到 backups 文件夹。");
  if (!confirmed) return;
  setStatus("正在创建备份...");
...
document.querySelector("#adminTabs").addEventListener("click", (event) => {
  ...
  if (button.dataset.panel === "media") refreshMedia(true).catch(() => {});
  if (button.dataset.panel === "backup") refreshBackups(true).catch(() => {});
});
...
document.addEventListener("click", (event) => {
  ...
  if (deleteMediaButton) deleteMediaFromAdmin(deleteMediaButton.dataset.deleteMedia).catch((error) => setStatus(error.message, "error"));
});
...
refreshMediaBtn.addEventListener("click", () => refreshMedia().catch((error) => setStatus(error.message, "error")));
createBackupBtn.addEventListener("click", () => createBackupFromAdmin().catch((error) => setStatus(error.message, "error")));
refreshBackupsBtn.addEventListener("click", () => refreshBackups().catch((error) => setStatus(error.message, "error")));
