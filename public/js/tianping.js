...
function resetSettleConfirm() {
  clearTimeout(settleConfirmTimer);
  els.manualSettleBtn.textContent = "结单";
}
...
els.manualSettleBtn.addEventListener("click", () => {
  if (els.manualSettleBtn.dataset.confirming !== "1") {
    els.manualSettleBtn.dataset.confirming = "1";
    els.manualSettleBtn.textContent = "再次点击确认结单";
    setMessage(els.scaleMessage, "结单会结束当前房间，请再点击一次确认。");
    settleConfirmTimer = setTimeout(() => {
      els.manualSettleBtn.dataset.confirming = "";
      resetSettleConfirm();
      setMessage(els.scaleMessage, "结单确认已取消。");
    }, 4500);
    return;
  }
  els.manualSettleBtn.dataset.confirming = "";
